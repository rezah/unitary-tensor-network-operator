...
def test_measure():
    ...
    ave,err = measure_pstr(Ulist,L,psi,analys_coefs=True)
    print "average: ", ave
    print "error: ", err
    print "True E: ", Egs

def test_Hvar():
    var = ising_variance(L)
    print var
...
def test_uhu():
    ...
    for i in range(64):
        c = pcoef[0][i]
        H_ = c*np.kron(p3b[i],np.eye(2**(L-3)))
        diag += np.diag(H_)
    
    for n in range(1,L-2):
        for i in range(256):
            c = pcoef[n][i]
            H_ = c*np.kron(np.kron(np.eye(2**(n-1)),p4b[i]),np.eye(2**(L-n-3)))
            diag += np.diag(H_)
    
    for i in range(64):
        c = pcoef[-1][i]
        H_ = c*np.kron(np.eye(2**(L-3)),p3b[i])
        diag += np.diag(H_)
